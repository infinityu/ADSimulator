 ...
 */
public class ConcreteExe {

	private HashMap<String, Integer> variableHashMap = new HashMap<String, Integer>();

	static final int NO_RIGHT_BRANKET = 0;
	private int pos = 0;
	private String[] expr;

	public ConcreteExe(HashMap<String, Integer> inputHashMap) {
		this.variableHashMap = new HashMap<String, Integer>(inputHashMap);
	}

	public void execute(String INPUT) {
		System.out.println("ConcreteExe->");
		...
		for (String statement : statements) {
			...
			Integer operandRight = parse(expr);
			variableHashMap.put(operand[0].trim(), operandRight);
		}
		Set<String> keySet = variableHashMap.keySet();
		for (String var : keySet) {
			System.out.println(var + " = " + variableHashMap.get(var));
		}
		System.out.println("----");
	}
	...
	public void setInitialVariableHashMap(
			HashMap<String, Integer> variableHashMap) {
		this.variableHashMap = new HashMap<String, Integer>(variableHashMap);
	}
}
