package simulator.view.treeview.views;
...
public class TraceView extends ViewPart {
	 ...
	 */
	public static final String ID = "simulator.view.treeview.views.TraceView";
	...
	static {
		...
		uml = "E:\\runtime-EclipseApplication\\" + diagram.getName() + ".uml";
		animation = new Animation(diagram);
		simulator = Psimulator.getInstance(uml, animation);
	}
	...
	class ViewContentProvider implements IStructuredContentProvider, ITreeContentProvider {
		...
		public ViewContentProvider(List<List<TreeObject>> record) {
			invisibleRoot = new TreeParent("");
			int index = 1;
			for (List<TreeObject> list : record) {
				TreeParent trace = new TreeParent("execution trace " + index++);
				for (TreeObject to : list) {
					trace.addChild(to);
				}
				invisibleRoot.addChild(trace);
			}
		}
	...
	private void showMessage(String message) {
		MessageDialog.openInformation(viewer.getControl().getShell(), "Tree View", message);
	}
	...
	public void updateUI(List<List<TreeObject>> list) {
		viewer.setContentProvider(new ViewContentProvider(list));
		viewer.expandAll();
	}
